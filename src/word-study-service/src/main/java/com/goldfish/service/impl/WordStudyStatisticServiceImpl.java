 ...
 * @author hellosscat
 * @since 2018-5-2
 *<p>  WordStudyStatisticservice实现</p>
 *
 */
@Service("wordStudyStatisticService")
public class WordStudyStatisticServiceImpl implements WordStudyStatisticService {

	private static final Logger logger = LoggerFactory.getLogger(WordStudyStatisticServiceImpl.class);
    ...
    public CommonResult<WordStudyStatistic> addWordStudyStatistic(WordStudyStatistic wordStudyStatistic) {
		...
		try {
			...
			logger.error("添加 WordStudyStatistic失败", e);
	...
	public CommonResult<WordStudyStatistic> updateWordStudyStatistic(WordStudyStatistic wordStudyStatistic) {
		...
		try {
			...
			logger.error("更新 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
	}
	...
	public CommonResult<WordStudyStatistic> deleteWordStudyStatistic(Long id) {
		...
		try {
			...
			logger.error("删除 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
    }
		...
		try {
			...
			logger.error("根据主键获取 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
	}
	...
	public CommonResult<List<WordStudyStatistic>> getAll() {
		...
		try {
			List<WordStudyStatistic> list = wordStudyStatisticManager.getAll();
			result.addDefaultModel("list", list);
			result.setSuccess(true);
		} catch (Exception e) {
			logger.error("取得所有 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
	}
	...
	public CommonResult<List<WordStudyStatistic>> getListByExample(WordStudyStatistic wordStudyStatistic) {
		...
		try {
			...
			logger.error("取得 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
	}
	...
	public CommonResult<WordStudyStatistic> getUnique(WordStudyStatistic wordStudyStatistic) {
		...
		try {
			...
			logger.error("根据example获取唯一 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
	}
	...
	public CommonResult<List<WordStudyStatistic>> getWordStudyStatisticByPage(PageQuery pageQuery) {
		...
		try {
			...
			logger.error("分页获取 WordStudyStatistic失败", e);
			result.setSuccess(false);
		}
		return result;
	}
