...
public class AjaxExamController extends BaseController {
    ...
    private ThreadPoolExecutor threadPool = ThreadPoolContext.getThreadPool(ThreadConstant.Exam, false);
     ...
     */
    /**
     *  TODO：此处生词熟词均按照熟词出题。因为MemoryLevel在Mybatis的XML里没带进来。但实际是按照studied字段来选择用户学过的单词来出题。
     *  为了避免问题。此处先这样写，不影响使用。
     */
    ...
    ExamVO doGetExam(Integer testArea, Integer questionNbr, String questionTypes, ModelMap context) {
        ...
        List<String> lessonIds = Arrays.asList(user.getLessonIds().split(","));
        Collections.shuffle(lessonIds);
        //2.根据用户信息和testArea查询单词列表
        WordStudy wordStudyQuery = new WordStudy();
        List<WordStudy> wordStudyList = new ArrayList<>();
        ...
        //防止出现用户的这个lesson根本没学过，导致出不来题
        for (String lessonString : lessonIds)
        {
            nowLessonId = Integer.valueOf(lessonString);
            wordStudyQuery.setLessonId(nowLessonId);
            wordStudyList = wordStudyService.getStudiedWords(wordStudyQuery).getDefaultModel();

            if (wordStudyList == null || wordStudyList.isEmpty()) {
                continue;
            }
            ...
            for (String str : questionTypesArray)
            {
                ...
                questionQuery.setLessonId(nowLessonId);
                questionList = questionService.getListByExample(questionQuery).getDefaultModel();
            ...
            if (ecList.size()+ceList.size()+lcList.size()+lwList.size() >= questionNbr*questionTypesArray.length)
            {
                break;
            }
        }
    ...
    @RequestMapping(value = "GetExam2", method = {RequestMethod.GET, RequestMethod.POST})
    public
    @ResponseBody
    ExamVO doGetExam2(Integer testArea, Integer questionNbr, String questionTypes, ModelMap context) {
        ExamVO examVO = new ExamVO();
        // 1.装填用户信息
        User user = this.getUserInfo();
        if (user == null) {
        ...
        //2.根据用户信息和testArea查询单词列表
        List<WordStudy> wordStudyList = this.getRandomStudyWords(testArea, user);
    ...
    private void getQuestions(final Integer questionNbr, String questionTypes, final List<WordStudy> wordStudyList, ExamVO examVO, User user) {
        try {
            ...
            for (String str : questionTypesArray) {
                ...
                threadPool.submit(getQuestionsTask);
            }
            for (Map.Entry<Integer, FutureTask<List>> entry : questionMap.entrySet()) {
                if (entry.getKey().equals(QuestionTypes.EN2CH.getNumber())) {
                    examVO.setDataEn2Ch(entry.getValue().get());
                    continue;
                }
                if (entry.getKey().equals(QuestionTypes.CH2EN.getNumber())) {
                    examVO.setDataCh2En(entry.getValue().get());
                    continue;
                }
                if (entry.getKey().equals(QuestionTypes.LISTEN2CH.getNumber())) {
                    examVO.setDataListen2Ch(entry.getValue().get());
                    continue;
                }
                if (entry.getKey().equals(QuestionTypes.LISTEN2WRITE.getNumber())) {
                    examVO.setDataListen2Write(entry.getValue().get());
                }
            }
            examVO.setCondition(0);
            examVO.setMsg("success");
            ...
            LogTypeEnum.EXCEPTION.error(e, "出题失败");
        }
    }
...
//        }
    ...
    private List<WordStudy> getRandomStudyWords(int testArea, User user) {
        WordStudy wordStudyQuery = new WordStudy();
        TestArea testArea1 = TestArea.getTestArea(testArea);
        wordStudyQuery.setMemoryLevel(testArea1.getCorrespondingMemoryLevel().getLevel());
        wordStudyQuery.setStudentId(user.getId().intValue());
    ...
    private List<QuestionVO> getVoFromQuestions(Question questionQuery,QuestionTypes questionType)
    {
        ...
        //使用HashSet来去重
        HashSet<String> vocCodeSet = new HashSet<>(questionLimit);
        for (Question q : questionList){
            if (!vocCodeSet.contains(q.getVocCode())) {
                vocCodeSet.add(q.getVocCode());
