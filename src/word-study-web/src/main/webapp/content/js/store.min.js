...
!function (f) {
  ...
  function isJSON(obj) {
    return "object" == typeof obj && "[object object]" === Object.prototype.toString.call(obj).toLowerCase() && !obj.length
  }
  ...
  function dealIncognito(storage) {
    ...
    try {
      ...
      storage.getItem(_KEY) === _VALUE && storage.removeItem(_KEY)
    }
    return storage
  }
  function Store() {
    return this instanceof Store ? void 0 : new Store
  }
  if (window.localStorage) {
    var store, storage = window.localStorage, even_storage = function () { };
    storage = dealIncognito(storage),
      Store.prototype = {
        set: function (key, val) {
          if (even_storage("set", key, val),
            key && !isJSON(key))
        ...
        get: function (key) {
          ...
          return "?" === key.charAt(0) ? this.has(key.substr(1)) : deserialize(storage.getItem(key))
        },
        clear: function () {
          return this.forEach(function (key, val) {
        ...
        keys: function () {
          var d = [];
          return this.forEach(function (k, list) {
            d.push(k)
          }),
            d
        },
        ...
        forEach: function (callback) {
          for (var i = 0; i < storage.length; i++) {
            ...
            if (callback(key, this.get(key)) === !1)
              break
          }
          return this
        },
        search: function (str) {
          for (var arr = this.keys(), dt = {}, i = 0; i < arr.length; i++)
            arr[i].indexOf(str) > -1 && (dt[arr[i]] = this.get(arr[i]));
          return dt
        },
        onStorage: function (cb) {
          return cb && isFunction(cb) && (even_storage = cb),
            this
        }
      },
      store = function (key, data) {
        ...
        if (2 === argm.length && "string" == typeof key) {
          if (!data)
          ...
          if (data && isFunction(data))
            return dt = null,
              ...
              dt ? store.set(key, dt) : store
        }
        if (2 === argm.length && isArray(key) && isFunction(data)) {
